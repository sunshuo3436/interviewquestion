...
func main() {
	...
	// 发送完整报文
	fmt.Println("Sending complete message (part 1)...")
	conn.Write(msg1Part1)
	time.Sleep(1 * time.Second) // 等待1秒

	fmt.Println("Sending complete message (part 2)...")
	conn.Write(msg1Part2)
	time.Sleep(1 * time.Second) // 等待1秒

	// 发送黏连报文
	fmt.Println("Sending sticky message...")
	conn.Write(msg2)
	time.Sleep(1 * time.Second) // 等待1秒

	// 发送分段报文
	fmt.Println("Sending fragmented message (part 1)...")
	conn.Write(msg3Part1)
	time.Sleep(500 * time.Millisecond) // 等待0.5秒

	fmt.Println("Sending fragmented message (part 2)...")
